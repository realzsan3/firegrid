...
# -*- coding: utf-8 -*-
"""
financial_life_cycle.py   v2025‑07‑13
-------------------------------------
• “积累期 ➜ 退休期” 全生命周期模拟
...
def simulate(args):
   ...
   # ---- 收尾：确定是否已退休 & 生成 summary ----
    retired_flag = retire_year is not None          # True / False

    if not retired_flag:
        retire_year = None                          # 保持显式 None
        # 未退休时，用“最终资产是否 ≥ 目标”来定义 safe
        safe = assets >= target

    summary = {
        "start_year": start_year,
        "retire_year": retire_year,  # 可能为 None
...
def parse_args():
    ...
    p.add_argument("-k", "--k", type=float, default=0.8, help="灵敏度系数 k")
